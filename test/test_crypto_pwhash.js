...
describe('PWHash argon2id', function() {
	it('should verify the generated hash with same password', function(done) {
		var password = Buffer.from('this is a test password', 'utf8');

		var out = sodium.crypto_pwhash_argon2id_str(
			password,
			sodium.crypto_pwhash_argon2id_OPSLIMIT_INTERACTIVE,
			sodium.crypto_pwhash_argon2id_MEMLIMIT_INTERACTIVE);
		assert(sodium.crypto_pwhash_argon2id_str_verify(out, password));
	...
	it('should not verify the generated hash with different passwords', function(done) {
		...
		assert(sodium.crypto_pwhash_argon2id_str_verify(out, badPassword)==false);
